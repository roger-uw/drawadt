...
newtype RenderUnicodeDrawT v l s fl nl nr fr a = 
  RenderUnicodeDrawT (Cxt -> Cxt -> Reader (UnicodeConfig s) (UnicodeDrawT v l s fl nl nr fr a))
...
instance (RenderUnicodeDrawTConstraints v l s) => DrawT (RenderUnicodeDrawT v l s) where
  lineT _ s = RenderUnicodeDrawT . const . const $ do
    wireH <- wireHSym <$> ask
    return $ UnicodeDrawT SL.empty SL.empty (SL.singleton . LL.cons wireH . LL.singleton . fromString $ s) SL.empty
  RenderUnicodeDrawT d1 +> RenderUnicodeDrawT d2 = RenderUnicodeDrawT $ \cxt cxtB -> do
    UnicodeDrawT fl1 nl1 nr1 fr1 <- d1 R (mergeCxt cxtB R)
    UnicodeDrawT fl2 nl2 nr2 fr2 <- d2 U (mergeCxt cxtB U)
    c <- ask
    let wireV = wireVSym c
        space = spaceSym c
        conn = if cxt == R then connRRSym c else connRSym c
    return $ UnicodeDrawT {
      farLeftT = fl1, 
      nearLeftT = nl1,
      nearRightT = nr1
          `SL.append` SL.map (LL.cons wireV . LL.tail) fr1 
          `SL.append` SL.map (LL.cons wireV) fl2
          `SL.append` SL.map (LL.cons wireV) nl2
          `SL.append` SL.singleton (LL.cons conn (SL.head nr2)),
      farRightT = SL.map (LL.cons space) (SL.tail nr2)
          `SL.append` SL.map (LL.cons space) fr2
    }
  RenderUnicodeDrawT d1 <+ RenderUnicodeDrawT d2 = RenderUnicodeDrawT $ \cxt cxtB -> do
    UnicodeDrawT fl1 nl1 nr1 fr1 <- d1 U (mergeCxt cxtB U)
    UnicodeDrawT fl2 nl2 nr2 fr2 <- d2 L (mergeCxt cxtB L)
    c <- ask
    let wireV = wireVSym c
        space = spaceSym c
        conn = if cxt == L then connLLSym c else connLSym c
    return $ UnicodeDrawT {
      farLeftT =      SL.map (LL.cons space) fl1
          `SL.append` SL.map (LL.cons space) nl1,
      nearLeftT =     LL.cons conn (SL.head nr1)
          `SL.cons`   SL.map (LL.cons wireV) (SL.tail nr1)
          `SL.append` SL.map (LL.cons wireV) fr1
          `SL.append` SL.map (LL.cons wireV . LL.tail) fl2
          `SL.append` nl2,
      nearRightT = nr2,
      farRightT = fr2
    }
...
instance (RenderUnicodeDrawTConstraints v l s) => BlockDrawT (RenderUnicodeDrawT v l s) where
  blockT _ (RenderUnicodeDrawT d) = RenderUnicodeDrawT $ \cxt cxtB -> do
    UnicodeDrawT fl nl nr fr <- d U (mergeCxt cxtB U)
    c <- ask
    let wireH = wireHSym c
        conv
          | cxtB == L = convLSym c
          | cxtB == R = convRSym c
          | cxtB == LR = convLRSym c
        ...
        space = spaceSym c
    return $ UnicodeDrawT {
      farLeftT =         SL.map (LL.cons space) fl
             `SL.append` SL.map (LL.cons space) nl,
      nearLeftT = SL.empty,
      nearRightT = SL.singleton (LL.cons wireH (LL.cons conv (SL.head nr))),
      farRightT =        SL.map (LL.cons space) (SL.tail nr)
      `       SL.append` SL.map (LL.cons space) fr
    }

runRenderUnicodeDrawT :: (RenderUnicodeDrawTConstraints v l s) =>
  UnicodeConfig s -> RenderUnicodeDrawT v l s fl nl nr fr a -> s
runRenderUnicodeDrawT c (RenderUnicodeDrawT d) = flattenUnicodeDrawT (runReader (d U U) c)
